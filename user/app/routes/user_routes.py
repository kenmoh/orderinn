...
from ..service.user_service import (
    CreateRoomService,
    UserService,
    create_staff_permission_group,
)
from ..schemas.user_schema import (
    AddStaffToOutletReturnSchema,
...
async def create_guest_user(data: CreateGuestUserSchema) -> GuestReturnSchema:
...
async def create_company_user(data: CreateUserSchema) -> UserReturnSchema:
    try:
...
@user_router.patch(
    "/{staff_id}/update-staff-permission", status_code=status.HTTP_202_ACCEPTED
)
async def create_staff_user(
...
async def create_profile(
    ...
    try:
        return await user_service.create_profile(data=data, current_user=current_user)
    except Exception as e:
...
async def create_permission_group(
    data: CreatePermissionGroupSchema, current_user: User = Depends(get_current_user)
) -> OutletSchema:
...
async def create_outlet(
    data: OutletSchema, current_user: User = Depends(get_current_user)
) -> OutletSchema:
    try:
...
@user_router.post(
    ...
    response_model=AddStaffToOutletReturnSchema,
)
async def add_staff_to_outlet(
    outlet_id: PydanticObjectId,
    data: AddStaffToOutletSchema,
    current_user: User = Depends(get_current_user),
):
    try:
        return await user_service.add_staff_to_outlet(
            outlet_id=outlet_id, staff=data, current_user=current_user
        )
    except Exception as e:
...
async def get_company_outlet(
    current_user: User = Depends(get_current_user)
) -> list[AddStaffToOutletReturnSchema]:
    try:
...
async def create_no_post_rooms(
    data: NoPostRoomSchema, current_user: User = Depends(get_current_user)
) -> NoPostRoomSchema:
    try:
        return await room_service.create_no_post_rooms(
            data=data, current_user=current_user
        )
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
