...
from ..service.item_service import ItemService, InventoryService
from ..schemas.item_schema import CreateItemReturnSchema, CreateItemSchema, InventorySchecma, ItemStockReturnSchema, ItemStockSchema
...
async def add_new_stock(
    item_id: PydanticObjectId, stock: ItemStockSchema, current_user: User = Depends(get_current_user)
...
@inventory_router.patch("/{item_id}/item-inventory", status_code=status.HTTP_202_ACCEPTED)
async def update_stock(
    item_id: PydanticObjectId, stock_id: PydanticObjectId, stock: ItemStockSchema, current_user: User = Depends(get_current_user)
) -> ItemStockReturnSchema:
    try:
